...
#if jiggars are present other than yellow area
kernelOpen = np.ones((5,5))
#if jiggars are present in yellow area
...
while True:
    ...
    #use HSV of yellow to detect only yellow colour
    ...
    if(len(conts)!=0):#draws the contours of that object which has the highest
        ...
        cv2.drawContours(flipped, b, -1, (0,225,0), 3)
        cv2.circle(flipped, west, 6, (0,0,225), -1)
        cv2.circle(flipped, east, 6, (0,0,225), -1)
        cv2.circle(flipped, north, 6, (0,0,225), -1)
        cv2.circle(flipped, south, 6, (0,0,225), -1)
        cv2.circle(flipped, (int(centre_x),int(centre_y)), 6, (0,0,225), -1)
